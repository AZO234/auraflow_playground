...
- 生成画像は 3_8_AuraFlow_generated に PNG + A1111 メタ付きで保存し、画面下段の
...
# AuraFlow 単一レーン: 既定 1024²、「2 人以上 (ワイド)」ON 時は横長 1216x832 (generate.py の many と同値)
DEFAULT_RES = 1024
WIDE_RES = (1216, 832)
...
class GenerateGUI:
    ...
    def _build_ui(self) -> None:
        ...
        # AuraFlow 単一レーン: 版フィルタ (SDXL/SD15/混合) ラジオは廃止
        ...
        # ControlNet (3_3_AuraFlow_ControlNet があれば)
        row += 1
    ...
    def _scan_assets(self) -> None:
        # checkpoint は all-in-one safetensors のみ (AuraFlow / Pony V7。GGUF 非対応)
        self.checkpoints = _list_safetensors(CHECKPOINT_DIR)
    ...
    def _filtered_checkpoints(self) -> list[Path]:
        """checkpoint リスト (AuraFlow 単一レーン、フィルタなし)。"""
        return list(self.checkpoints)

    def _populate_cn_listbox(self) -> None:
        """ControlNet listbox を self.controlnets (3_3_AuraFlow_ControlNet の中身) で 1 回だけ埋める。"""
    ...
    def _open_settings(self) -> None:
        ...
        ttk.Checkbutton(gen_frame, text="Pony V7 (cfg1.5/euler_cfg_pp/clip-2/score)",
                        variable=self.pony_var).grid(row=1, column=4, columnspan=2, sticky="w", padx=(12, 2))
    ...
    def _worker(self, params: dict) -> None:
        ...
        try:
            ...
            checkpoint_data = {}
        try:
            auraflow_lora_subjects = load_auraflow_lora_subjects()
        except Exception:
            auraflow_lora_subjects = {}
        ...
        while not self._stop_flag:
            ...
            workflow = build_workflow_txt2img(
                ...
                pony=pony,
                # all-in-one 同梱 VAE を使う (外部 VAE 上書きは GUI では未対応)
                vae_override=None,
            )
    ...
    def _do_upscale_sync(
        ...
        """1 枚を Real-ESRGAN x4 アップスケールして 3_9_AuraFlow_upscaled に保存。
