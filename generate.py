#!/usr/bin/env python3
"""generate.py - ComfyUI HTTP API 経由で AuraFlow 画像を連続生成する CLI。
...
実行モデル:
    ...
    各 source ループで:
        ...
        ComfyUI に投入 → 完成画像を fetch → A1111 メタ付き PNG で 3_8_AuraFlow_generated に保存
...
特徴:
    - AuraFlow は通常の CFG モデル → KSampler cfg=3.5、negative プロンプトが実効 (FluxGuidance 不使用)
    - checkpoint は 3_1_AuraFlow_checkpoint からランダム (all-in-one を CheckpointLoaderSimple で一括ロード)
    - base AuraFlow / Pony V7 両対応 (--pony で euler_cfg_pp / cfg=1.5 / clip skip -2 / score タグ)
    - LoRA / ControlNet / ADetailer / Hires Fix / upscale 対応、Ctrl+C でループ停止
    - A1111 互換メタを 3_8_AuraFlow_generated/{YYYYMMDDHHMMSS}.png に書き込み
"""
...
# --- AuraFlow 単一レーン dir (extra_model_paths.yaml 生成にも使う) ---
CHECKPOINT_DIR   = ROOT / "3_1_AuraFlow_checkpoint"   # AuraFlow base (all-in-one)
LORA_DIR         = ROOT / "3_2_AuraFlow_LoRA"
CONTROLNET_DIR   = ROOT / "3_3_AuraFlow_ControlNet"
...
def write_extra_model_paths() -> bool:
    """playground の AuraFlow model dir を ComfyUI に認識させる extra_model_paths.yaml を
    dir 定数から自動生成する。
    ...
    content = (
        ...
        "# playground の AuraFlow model dir を ComfyUI に登録。\n"
        "comfyui_playground:\n"
        f"    base_path: {ROOT.as_posix()}/\n"
        "    is_default: true\n"
        f"    checkpoints: {CHECKPOINT_DIR.name}/\n"
        # transformer 単体 checkpoint を UNETLoader (分離ローダー経路) から読めるよう同 dir を登録
        ...
        f"    vae: {VAE_DIR.name}/\n"
        # etc/ を補助ステージング dir として登録 (分離経路の pile-T5 / SDXL VAE をここに置ける)
        "comfyui_playground_etc:\n"
        f"    base_path: {ROOT.as_posix()}/\n"
        "    text_encoders: etc/\n"
        "    vae: etc/\n"
    )
...
def _pids_listening_on_port(port: int) -> set[int]:
    """指定ポートを LISTEN しているプロセスの PID 集合を返す (起動方法に依存せず確実)。"""
    import psutil
    pids: set[int] = set()
    try:
...
def _find_comfyui_processes() -> list:
    """ComfyUI server プロセスを返す。判定は2系統で取りこぼしを防ぐ:
      1) COMFY_PORT を LISTEN しているプロセス (起動方法・cwd 不問で確実)
      2) cmdline に main.py --listen を含むプロセス (フォールバック)
    別プロジェクト (cmdline に 'ComfyUI' 文字列を含まない .venv 起動等) も 1) で捕捉する。"""
    import psutil
    seen: set[int] = set()
    result = []
    port_pids = _pids_listening_on_port(COMFY_PORT)
    for proc in psutil.process_iter(["pid", "cmdline", "name"]):
        try:
            pid = proc.info.get("pid")
            cmdline = proc.info.get("cmdline") or []
            joined = " ".join(str(a) for a in cmdline)
            is_comfy_cmd = "main.py" in joined and "--listen" in joined
            if (pid in port_pids or is_comfy_cmd) and pid not in seen:
                seen.add(pid)
                result.append(proc)
...
def kill_comfyui_server(timeout: float = 30.0) -> None:
    """COMFY_PORT 上の ComfyUI server を kill し、ポートが空くまで待つ (シングルトン担保)。"""
    import psutil
...
def comfyui_serves_our_models() -> bool:
    """動いている ComfyUI が本プロジェクトの checkpoint を配信しているか判定する。
    3_1_AuraFlow_checkpoint の実ファイルが 1 つも ComfyUI のリストに無ければ、別プロジェクトの
    ComfyUI が同ポートを占有している (foreign instance) とみなす。比較不能なら True (誤再起動回避)。"""
    ours = {p.name for p in CHECKPOINT_DIR.glob("*.safetensors")} | {p.name for p in CHECKPOINT_DIR.glob("*.gguf")}
    if not ours:
        return True
    names: set[str] = set()
    try:
        info = _http_get_json(f"{COMFY_BASE}/object_info/CheckpointLoaderSimple")
        names |= set(info["CheckpointLoaderSimple"]["input"]["required"]["ckpt_name"][0])
    except Exception:
        return True  # object_info 取れない → 判定保留 (再起動はしない)
    try:
        uinfo = _http_get_json(f"{COMFY_BASE}/object_info/UNETLoader")
        names |= set(uinfo["UNETLoader"]["input"]["required"]["unet_name"][0])
    except Exception:
        pass
    return bool(ours & names)
...
def ensure_comfyui_arch(arch: str, force_restart: bool = False) -> None:
    ...
    起動時設定を読み直させたいケース)。別プロジェクトの ComfyUI が同ポートを占有している場合も
    強制再起動して本プロジェクトのモデルを配信させる。`arch` ∈ {'cuda', 'cpu'}。
    """
    ...
    if cur is None:
        ...
        return
    # 動いている ComfyUI が別プロジェクトのもの (本プロジェクトの checkpoint 非配信) なら強制再起動
    foreign = (cur == arch and not force_restart and not comfyui_serves_our_models())
    if cur == arch and not force_restart and not foreign:
        # 一致 + 再起動不要 + 自分のモデルを配信中 → 何もしない
        return
    if foreign:
        reason = L("別プロジェクトの ComfyUI が同ポートを占有 → 乗っ取り再起動",
                   "a different project's ComfyUI holds this port → taking over")
    else:
...
def build_workflow_txt2img(
    *,
    ...
    """txt2img / img2img の workflow JSON を組み立てる (AuraFlow / Pony V7)。

    AuraFlow は通常の CFG モデルなので KSampler の cfg をそのまま使い (既定 3.5)、
    **negative プロンプトが実効** (FluxGuidance ノードは使わない)。all-in-one checkpoint
    (model+clip+vae 同梱) を CheckpointLoaderSimple がそのまま MODEL/CLIP/VAE として返す。
    pony=True のときは CLIPSetLastLayer(stop_at_clip_layer=-2) を挿入する (Pony V7 推奨)。

    ローダー 2 系統:
      - all-in-one (既定): CheckpointLoaderSimple 一本で MODEL/CLIP/VAE。
      - 分離 (unet_only=True): transformer 単体 checkpoint 用。UNETLoader(checkpoint) +
        CLIPLoader(clip_name=pile-T5、type は ComfyUI が weights から自動判別=AuraT5) +
        VAELoader(vae_override=SDXL系 VAE)。clip_name と vae_override が必須。
        text encoder は ComfyUI/models/text_encoders、unet は diffusion_models 登録 dir から解決。
    ...
    **必ず base 側** (AuraFlow ネイティブ ~1MP、1024 等) を渡す。Hires Fix off のときは
    ...
    if unet_only:
        # 分離ローダー: transformer 単体 AuraFlow (model のみ) 用。
        # pile-T5 は weights から AuraT5 と自動判別されるので CLIPLoader type は実質任意。
        if not clip_name:
            raise ValueError("unet_only には clip_name (pile-T5 text encoder) が必要です")
        if not vae_override:
            raise ValueError("unet_only には vae_override (SDXL系 VAE) が必要です")
        workflow: dict = {
            "1":  {"class_type": "UNETLoader", "inputs": {"unet_name": checkpoint, "weight_dtype": unet_dtype}},
            "52": {"class_type": "CLIPLoader", "inputs": {"clip_name": clip_name, "type": clip_type}},
        ...
        # AuraFlow / Pony V7 は all-in-one 配布が主流なのでこちらが既定。
        workflow = {
            "1": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": checkpoint}},
        }
        model_ref = ["1", 0]
        clip_ref  = ["1", 1]
        # VAE: --vae 明示時は VAELoader (node 50) を経由、未指定は checkpoint 同梱 VAE
        if vae_override:
    ...
    # Pony V7 は CLIP skip -2 推奨 → CLIPSetLastLayer を text encode の直前に挿入
    if pony:
        workflow["55"] = {
    ...
    if init_image:
        ...
        # AuraFlow は SDXL 系の 4ch latent → EmptyLatentImage を使う (Flux の 16ch とは異なる)。
        workflow["4"] = {
    ...
    # AuraFlow は通常 CFG モデル → positive/negative をそのまま KSampler に渡す (FluxGuidance なし)。
...
def load_auraflow_lora_subjects() -> dict[str, str]:
    ...
    if not AURAFLOW_LORA_HINT_TOML.exists():
        return {}
    try:
        data = tomllib.loads(AURAFLOW_LORA_HINT_TOML.read_text(encoding="utf-8"))
    except Exception as e:
...
def pick_controlnet(style: str, fixed_name: Optional[str] = None,
    ...
    if force_openpose:
        ...
        if not matched:
            raise SystemExit(
                L("--pose 指定だが 3_3_AuraFlow_ControlNet/ に openpose 系 ControlNet が見つかりません "
                  "(stem に 'openpose' / '_pose' / 末尾 'pose' を含むファイルを配置)",
                  "--pose specified but no openpose ControlNet found in 3_3_AuraFlow_ControlNet/ "
...
# --------------------------------------------------------------------------- #
# VAE の解決
# AuraFlow は all-in-one checkpoint に VAE 同梱なので既定は同梱 VAE を使う。
# 3_4_AuraFlow_VAE に VAE を置いて `--vae NAME` を渡すと VAELoader で明示使用する。
# --------------------------------------------------------------------------- #
def resolve_vae(name: Optional[str]) -> Optional[str]:
    """`--vae` を解決。3_4_AuraFlow_VAE 配下に該当ファイルがあればその名前を返す
    (ComfyUI は extra_model_paths の vae path で解決)。無ければ None (= 同梱 VAE)。"""
    if not name:
        return None
    cand = VAE_DIR / name
    if not cand.exists() and not name.endswith(".safetensors"):
        cand = VAE_DIR / f"{name}.safetensors"
    if cand.exists():
        return cand.name
    print(L(f"  [vae][warn] --vae {name} が {VAE_DIR.name}/ に無い → 同梱 VAE を使用",
            f"  [vae][warn] --vae {name} not found in {VAE_DIR.name}/ → using bundled VAE"), flush=True)
    return None
...
def checkpoint_is_unet_only(path: Path) -> bool:
    ...
    判定不能 (.gguf 等) は False (= all-in-one 扱い)。"""
    ...
    try:
        ...
        for k in hdr:
            ...
            if k.startswith("text_encoders."):
                ...
                has_vae = True
        # all-in-one は両方を内包。どちらか欠ければ transformer 単体とみなす。
        return not (has_te and has_vae)
    except Exception:
        return False
...
def pick_checkpoint(
    ...
    """checkpoint を抽選 (`checkpoint.toml` 連動、AuraFlow 単一プール)。
...
def prepare_workflow_prompt(
    ...
    *,
    ...
    subjects = auraflow_lora_subjects or {}
...
def get_prompt_for_iteration(
    ...
    if mode == "sentence":
        ...
        if pony:
            ...
            if pony_prefix:
                positive = normalize_emphasis(pony_prefix) + ", " + positive
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=L("ComfyUI HTTP API 経由で AuraFlow 画像を連続生成する",
                      "Continuously generate AuraFlow images via ComfyUI HTTP API")
    )
    ...
    ap.add_argument("--png", type=str, default=None,
                    help=L("画質アップ refine 用 PNG (1_0_prompts/ 配下 or 絶対パス)。"
                           ...
                           "Redraws the image via AuraFlow img2img (single image, then exits)"))
    ...
    ap.add_argument("--pose", type=str, default=None,
                    help=L("openpose 用 ソース PNG (絶対パス or 1_0_prompts/NAME)。"
                           ...
                           "(3_3_AuraFlow_ControlNet/ に stem に 'openpose'/'_pose' を含むファイルが必要)。"
                           ...
                           "(requires a file with 'openpose'/'_pose' in its stem in 3_3_AuraFlow_ControlNet/). "
    ...
    ap.add_argument("--gear", choices=["low", "high"], default="high",
                    help=L("low=ラフ (steps 20) / high=本番 (steps 28、既定)。AuraFlow 単一パス",
                           "low=rough (steps 20) / high=production (steps 28, default). AuraFlow single-pass"))
    ...
    ap.add_argument("--cfg-scale", type=float, default=None,
                    help=L("KSampler の CFG (未指定: base AuraFlow=3.5 / --pony=1.5。negative が実効)",
                           "KSampler CFG (default: base AuraFlow=3.5 / --pony=1.5. negative is effective)"))
    ap.add_argument("--pony", action="store_true",
                    help=L("Pony V7 推奨設定に切替 (euler_cfg_pp / cfg=1.5 / steps=30 / CLIP skip -2 / "
                           "prompt.toml の pony_prefix を positive 先頭に付加)",
                           "switch to Pony V7 recommended settings (euler_cfg_pp / cfg=1.5 / steps=30 / "
                           "CLIP skip -2 / prepend prompt.toml pony_prefix to positive)"))
    ...
    ap.add_argument("--sampler", type=str, default=None,
                    help=L("KSampler の sampler (未指定: base AuraFlow=euler / --pony=euler_cfg_pp)",
                           "KSampler sampler (default: base AuraFlow=euler / --pony=euler_cfg_pp)"))
    ap.add_argument("--scheduler", type=str, default=None,
                    help=L("KSampler の scheduler (未指定: sgm_uniform)",
    ...
    ap.add_argument("--upscale", action=argparse.BooleanOptionalAction, default=None,
                    help=L("Real-ESRGAN x4 アップスケール (3_9_AuraFlow_upscaled に出力)。"
                           "既定: gear high で ON / gear low で OFF。明示すれば上書き",
                           "Real-ESRGAN x4 upscale (output to 3_9_AuraFlow_upscaled). "
    ...
    ap.add_argument("--adetailer", action=argparse.BooleanOptionalAction, default=None,
                    help=L("ADetailer (顔/手 YOLO inpainting)。既定 OFF (AuraFlow では基本不要、8GB では最重)。"
                           "必要時のみ --adetailer。手/顔/身体は prompt で肯定文指定推奨",
                           "ADetailer (face/hand YOLO inpainting). Default OFF (rarely needed for AuraFlow, heaviest on 8GB). "
    ...
    ap.add_argument("--clip-type", type=str, default="stable_diffusion",
                    help=L("CLIPLoader の type (既定 stable_diffusion)。pile-T5 は weights から "
                           ...
                           "CLIPLoader type (default stable_diffusion). pile-T5 is auto-detected as AuraT5 "
                           "from its weights, so this rarely needs changing"))
    ap.add_argument("--unet-dtype", type=str, default="default",
                    choices=["default", "fp8_e4m3fn", "fp8_e4m3fn_fast", "fp8_e5m2"],
                    help=L("分離経路 UNETLoader の weight_dtype (既定 default)。VRAM 節約に fp8 系も可",
                           "weight_dtype for UNETLoader in the separate path (default). fp8 variants save VRAM"))
    ...
    # AuraFlow / Pony V7 の既定値解決 (未指定なら mode に応じて埋める。明示は尊重)。
    #   base AuraFlow: cfg=3.5 / euler / sgm_uniform
    #   Pony V7      : cfg=1.5 / euler_cfg_pp / sgm_uniform / CLIP skip -2 / score タグ
    if args.cfg_scale is None:
        args.cfg_scale = 1.5 if args.pony else 3.5
    if args.sampler is None:
        args.sampler = "euler_cfg_pp" if args.pony else "euler"
    if args.scheduler is None:
        args.scheduler = "sgm_uniform"
    ...
    # steps: base AuraFlow=low20/high28、Pony V7 は high で 30 を推奨
    steps = {"low": 20, "high": 30 if args.pony else 28}[args.gear]
    ...
    # ADetailer は AuraFlow では基本不要 (顔/手は高精細にネイティブ生成。8GB では再サンプリングが最重) →
    ...
    mode_label = "Pony V7" if args.pony else "AuraFlow"
    print(f"=== generate.py ({mode_label}) ===")
    print(f"prompt mode: {args.prompt}  gear: {args.gear} (steps={steps})  "
          f"cfg: {args.cfg_scale}  sampler: {args.sampler}/{args.scheduler}  arch: {args.arch}  "
          f"upscale: {args.upscale}  adetailer: {args.adetailer}  "
          f"hires_fix: {args.hires_fix}")

    print(f"\n--- tensors triage ---")
    counts = check_tensors()
    print(f"  AuraFlow: ckpt={counts['checkpoint']} LoRA={counts['lora']} "
    ...
    # --vae 指定時のみ 3_4_AuraFlow_VAE の VAE を VAELoader で使用 (未指定は同梱 VAE)
    vae_name: Optional[str] = resolve_vae(args.vae)
    if vae_name:
        print(L(f"  [vae] VAE 明示使用: {vae_name}",
                f"  [vae] explicit VAE: {vae_name}"), flush=True)
    ...
    while not stop["flag"]:
        try:
            ...
            if is_refine:
                ...
                print(L(f"  [refine] {src_png.name} を AuraFlow img2img で画質アップ "
                        f"(denoise {args.refine_denoise})",
                        f"  [refine] quality-up {src_png.name} via AuraFlow img2img "
                        f"(denoise {args.refine_denoise})"), flush=True)
            else:
                pipeline_label = "Pony V7 single-pass" if args.pony else "AuraFlow single-pass"

            # ---- 単一パス stage ----
            gen_width, gen_height = resolution(many)
            if is_refine:
                # 元 PNG のアスペクトを保ったまま ~1MP (AuraFlow ネイティブ) にスケール
            ...
            # ControlNet 抽選 (3_3_AuraFlow_ControlNet があれば。refine 段は除外)
            ...
            if args.dump_workflow or args.dump_only:
                kind = "refine" if is_refine else "auraflow_single"
            ...
            outputs = result.get("outputs", {})
            # node 7 = 通常解像度 (3_8_AuraFlow_generated)
            ...
            # node 14 = アップスケール後 (3_9_AuraFlow_upscaled)
            if upscale_model_name:
