...
方針 (AuraFlow 単一レーン):
  ...
  - **LoRA**: 3_1_AuraFlow_checkpoint の代表 AuraFlow ベースを自動選択し、LoRA + トリガー語 (ss_tag_frequency) で 1 枚。
...
CATEGORIES_FILE = ROOT / "LoRA_preview.toml"             # [AuraFlow_categories] / [AuraFlow_prompts]
...
def tensor_family(path: Path) -> str:
    ...
    判定不能は "auraflow"。"""
    return _family_from_name(path.stem) or "auraflow"
...
def gather(dirs: list[Path]) -> list[Path]:
    ...
    for d in dirs:
        if d.is_dir():
            ...
            out += [p for p in d.glob("*.gguf") if p.is_file()]   # .gguf も収集 (非対応だが)
    return sorted(out, key=lambda p: p.name.lower())


def build_family_bases(overrides: dict[str, Optional[str]]) -> dict[str, Path]:
    """family → 代表ベース checkpoint。全 LoRA を AuraFlow ベースで焼く。
    family 別に先頭を代表に置き、"auraflow"(全体の先頭)を必ずフォールバックとして持つ。
    CLI 上書き (--base-NAME) があれば最優先。"""
    checkpoints = gather([CHECKPOINT_DIR])
    bases: dict[str, Path] = {}
    if checkpoints:
        bases["auraflow"] = checkpoints[0]
...
def base_for_lora(lora: Path, bases: dict[str, Path]) -> Optional[Path]:
    """LoRA に使う AuraFlow ベースを返す。family 一致を優先、無ければ "auraflow"(全体先頭)。"""
    fam = tensor_family(lora)
    return bases.get(fam) or bases.get("auraflow") or (next(iter(bases.values())) if bases else None)
...
def build_positive(base_positive: str, family: str, trigger: str = "") -> str:
    # AuraFlow: scaffold + trigger を連結 (Pony の score タグは build_workflow 側 pony で付与)。
    parts = [base_positive]
    if trigger:
...
def res_for(version: str = "auraflow") -> tuple[int, int]:
    return (832, 1216)   # AuraFlow 縦構図 (単一被写体, ~1MP)
...
def load_preview_config(template_path: Path = PREVIEW_SETTINGS_TOML,
    ...
    戻り値:
      ...
      cats_by_ver:    {"auraflow": {stem→cat}}   (AuraFlow_categories)
      prompts_by_ver: {"auraflow": {stem→prompt}} (AuraFlow_prompts)
    """
    import tomllib
    ...
    if template_path.exists():
        try:
            ...
            pass
    # AuraFlow 単一レーン: cats/prompts は "auraflow" キー 1 本 (build_job が version="auraflow" で引く)
    cats_by_ver = {"auraflow": {}}
    prompts_by_ver = {"auraflow": {}}
    if lora_preview_path.exists():
        try:
            lp = tomllib.loads(lora_preview_path.read_text(encoding="utf-8"))
            cats_by_ver["auraflow"] = {str(k): str(v) for k, v in (lp.get("AuraFlow_categories") or {}).items()}
            prompts_by_ver["auraflow"] = {str(k): str(v) for k, v in (lp.get("AuraFlow_prompts") or {}).items() if str(v).strip()}
        except Exception:
            pass
...
def write_preview_categories(guess: bool = False,
    ...
    """全 LoRA を AuraFlow_categories に書き出す (既存 cats/prompts は保持)。
    ...
    if lora_preview_path.exists():
        try:
            ...
            data = {}
    existing = dict(data.get("AuraFlow_categories") or {})
    new_cats = {p.stem: existing.get(p.stem)
                   or (guess_category(p.stem) if guess else "ware")
                for p in sorted(LORA_DIR.glob("*.safetensors"))}
    data["AuraFlow_categories"] = dict(sorted(new_cats.items(), key=lambda kv: kv[0].lower()))
    if "AuraFlow_prompts" not in data:
        data["AuraFlow_prompts"] = {}
    ordered = {k: data[k] for k in ("AuraFlow_categories", "AuraFlow_prompts") if k in data}
    with open(lora_preview_path, "wb") as f:
        tomli_w.dump(ordered, f)
    print(L(f"{lora_preview_path.name} を書き出し: AuraFlow {len(ordered.get('AuraFlow_categories', {}))} 件",
            f"wrote {lora_preview_path.name}: AuraFlow {len(ordered.get('AuraFlow_categories', {}))} entries"))


def save_preview_entry(stem: str, version: str, category: str, custom_prompt: str,
                       lora_preview_path: Path = CATEGORIES_FILE) -> None:
    """1 件分の category + custom prompt を AuraFlow_{categories,prompts} に書き込む (version 引数は無視)。"""
    import tomllib
    import tomli_w
    data: dict = {}
    if lora_preview_path.exists():
        try:
            ...
            data = {}
    cats = dict(data.get("AuraFlow_categories") or {})
    prompts = dict(data.get("AuraFlow_prompts") or {})
    cats[stem] = category
    ...
    data["AuraFlow_categories"] = dict(sorted(cats.items(), key=lambda kv: kv[0].lower()))
    data["AuraFlow_prompts"] = dict(sorted(prompts.items(), key=lambda kv: kv[0].lower()))
    with open(lora_preview_path, "wb") as f:
        tomli_w.dump(data, f)
...
def render(*, checkpoint_name: str, loras, positive: str, negative: str, version: str,
           ...
           client_id: str, pony: bool = False) -> Optional[bytes]:
    w, h = res_for(version)
    wf = build_workflow_txt2img(
        ...
        loras=loras, filename_prefix="preview", pony=pony,
    )
...
def render_multi(*, checkpoint_name: str, loras, positive: str, negative: str, version: str,
    ...
    for s in seeds:
        data = render(checkpoint_name=checkpoint_name, loras=loras, positive=positive,
                      ...
                      sampler=sampler, scheduler=scheduler, client_id=client_id, pony=pony)
        if data:
            shots.append(data)
    if not shots:
        return None
...
def regenerate(path: Path, *, seed: Optional[int] = None, steps: int = 20, cfg: float = 3.5,
    ...
    data = render_multi(checkpoint_name=ckpt_name, loras=loras, positive=positive,
                        ...
                        cfg=cfg, sampler=sampler, scheduler=scheduler, pony=pony,
...
def main() -> None:
    ...
    ap.add_argument("--cfg", type=float, default=None,
                    help=L("CFG (未指定: base=3.5 / --pony=1.5)", "CFG (default: base=3.5 / --pony=1.5)"))
    ...
    ap.add_argument("--sampler", type=str, default=None,
                    help=L("sampler (未指定: base=euler / --pony=euler_cfg_pp)",
                           "sampler (default: base=euler / --pony=euler_cfg_pp)"))
    ap.add_argument("--scheduler", type=str, default="sgm_uniform")
    ...
    # base AuraFlow / Pony V7 の既定値解決 (未指定なら mode に応じて埋める)
    if args.cfg is None:
        args.cfg = 1.5 if args.pony else 3.5
    if args.sampler is None:
        args.sampler = "euler_cfg_pp" if args.pony else "euler"

    cat_file = Path(args.categories)
    if args.init_categories:
        # 3_2_AuraFlow_LoRA をスキャンして AuraFlow_categories を初期化 (既存手編集は保持)
    ...
    if args.files:
        ...
        for f in args.files:
            p = Path(f)
            try:
                out = regenerate(p, extra=args.extra, steps=args.steps, cfg=args.cfg,
                                 sampler=args.sampler, scheduler=args.scheduler, pony=args.pony,
    ...
    bases = build_family_bases({"auraflow": args.base})
    ...
    for idx, (kind, path) in enumerate(targets, 1):
        ...
        try:
            ...
            data = render_multi(checkpoint_name=ckpt_name, loras=loras, positive=positive,
                                ...
                                client_id=client_id, pony=args.pony)
        except Exception as ex:
