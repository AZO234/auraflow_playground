...
AuraFlow 出力 dir (3_8_AuraFlow_generated / 3_9_AuraFlow_upscaled) を再帰走査し、
A1111 互換メタ (parameters chunk) を読み取ってサムネイル + メタ情報の一覧を表示する。
名前 / 時刻 / アーキ(AuraFlow) / Model / Size でソート、テキスト検索でフィルタ、
...
アーキ判定は parameters の `Pipeline` フィールド ("AuraFlow") で行う。
...
# 閲覧対象の既定ディレクトリ (AuraFlow generated / upscaled を再帰走査して PNG を集める)
# `--dir PATH [PATH...]` 指定時はこの既定が差し替えられる (main() 参照)
VIEW_DIRS = [
    ROOT / "3_8_AuraFlow_generated",
    ROOT / "3_9_AuraFlow_upscaled",
]
...
PAD = 6
# アーキ表示色 (caption の文字色)
ARCH_COLOR = {"AuraFlow": "#6a1b9a", "?": "#777777"}
...
def detect_arch(params: dict) -> str:
    """parameters の Pipeline から AuraFlow 生成かを判定する (AuraFlow 単一レーン)。

    Pipeline 例: 'AuraFlow single-pass' / 'Pony V7 single-pass' / 'refine (...)'。
    AuraFlow/Pony で生成したものは "AuraFlow"、判定不能は "?"。
    """
    pl = (params.get("Pipeline") or "").upper()
    if "AURAFLOW" in pl or "PONY" in pl:
        return "AuraFlow"
    # Pipeline 不在 (手動配置や旧データ) は判定不能。refine 等 Pipeline があれば AuraFlow 扱い
    return "AuraFlow" if pl else "?"
...
def run_list(directories) -> None:
    ...
    n_af = sum(e.arch == "AuraFlow" for e in entries)
    print(L(f"  AuraFlow: {n_af} / 不明: {len(entries) - n_af}",
            f"  AuraFlow: {n_af} / unknown: {len(entries) - n_af}"))
...
def run_gui(directories) -> None:
    ...
    class GalleryApp(tk.Tk):
        ...
        def _build_body(self) -> None:
            ...
            self.tree.tag_configure("AuraFlow", foreground="#6a1b9a")
