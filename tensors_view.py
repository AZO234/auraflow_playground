...
表示するもの:
  - テキスト: 種別(base/lora/embedding/controlnet)、系統(auraflow/flux/SDXL/SD15)判定、
...
使い方:
    python tensors_view.py --dir 3_2_AuraFlow_LoRA          # GUI を起動
    python tensors_view.py --dir 3_2_AuraFlow_LoRA --list    # GUI なしで一覧を標準出力
"""
...
def detect_version(tensors: dict, meta: dict) -> str:
    """auraflow / flux / SD15 / SDXL を判定。メタ優先 → テンソルの shape ヒューリスティク。不明は '?'。"""
    arch = f"{meta.get('modelspec.architecture', '')} {meta.get('ss_base_model_version', '')} {meta.get('ss_network_module', '')}".lower()
    keys = list(tensors.keys())
    # AuraFlow (Pony V7 含む): メタ (auraflow) or AuraFlow DiT 署名 (double_layers/single_layers/cond_seq_linear)
    if "auraflow" in arch or any(("double_layers" in k or "single_layers" in k
                                  or "cond_seq_linear" in k or "init_x_linear" in k) for k in keys):
        return "auraflow"
...
def load_preview_meta() -> tuple[dict, dict, list]:
    ...
    AuraFlow 単一レーン: cats_by_ver / prompts_by_ver は {"auraflow": {stem→...}} (AuraFlow_categories/AuraFlow_prompts)。"""
    import tomllib
    cats_by_ver: dict = {"auraflow": {}}
    prompts_by_ver: dict = {"auraflow": {}}
    ...
    if PREVIEW_TOML.exists():
        try:
            ...
            cats_by_ver["auraflow"] = {str(k): str(v) for k, v in (data.get("AuraFlow_categories") or {}).items()}
            prompts_by_ver["auraflow"] = {str(k): str(v) for k, v in (data.get("AuraFlow_prompts") or {}).items()}
        except Exception:
            pass
...
def save_preview_meta(stem: str, version: str, category: str, custom_prompt: str) -> None:
    """LoRA_preview.toml の AuraFlow_{categories,prompts}[stem] を更新 (version 引数は無視、AuraFlow単一)。"""
    import tomllib
    import tomli_w
    data: dict = {}
    if PREVIEW_TOML.exists():
...
def set_preview_categories(stems_by_ver: dict, category: str) -> None:
    """version → [stem] の辞書を受け取り、AuraFlow_categories を 1 度の書き込みで一括更新 (version は無視)。"""
    import tomllib
    import tomli_w
    data: dict = {}
    if PREVIEW_TOML.exists():
...
def _entry_version(entry) -> str:
    """Entry の version。AuraFlow 単一レーンなので常に "auraflow" (LoRA_preview は AuraFlow_ セクションのみ)。"""
    return "auraflow"
    ...
    class TensorViewApp(tk.Tk):
        ...
        def _build_toolbar(self) -> None:
            ...
            base_cb = ttk.Combobox(bar, textvariable=self.base_var, width=6, state="readonly",
                                   values=["All", "auraflow", "flux", "sdxl", "sd15", "?"])
