#!/usr/bin/env python3
"""dist_tensors.py - `2_0_tensors` 受入トレーのテンソルを分類して各 dir へ振り分ける CLI (AuraFlow)。
...
処理内容 (idempotent、何度実行しても安全):
    ...
    - 系統 (auraflow / flux1 / flux2 / sdxl / sd15 / unknown) を判定して振り分け:
        AuraFlow (Pony V7 含む):
                     base → 3_1_AuraFlow_checkpoint / LoRA → 3_2_AuraFlow_LoRA /
                     ControlNet → 3_3_AuraFlow_ControlNet / VAE → 3_4_AuraFlow_VAE /
                     Embedding → 3_5_AuraFlow_Embedding
        それ以外 (Flux.1 / Flux.2 / SD15 / SDXL / 破損 / inpainting / 判別不能):
                     すべて 2_1_errortensors (本環境では非対応レーン)
    - キャッシュは tensors_cache.toml (path: {size, mtime, hash, kind, *_arch})

policy:
    - 3_3_AuraFlow_ControlNet / 2_1_errortensors は **scan しない** (手動配置を尊重、再分類しない)
    - 3_1/3_2/3_4/3_5 (AuraFlow base・LoRA・VAE・Embedding) は scan する (直接投入分も dedup + 再分類)
...
ERROR_DIR   = ROOT / "2_1_errortensors"   # 非対応 (Flux/SD15・SDXL/破損/inpainting/判別不能)
# --- AuraFlow レーン ---
CKPT_DIR    = ROOT / "3_1_AuraFlow_checkpoint"  # AuraFlow base (all-in-one)
LORA_DIR    = ROOT / "3_2_AuraFlow_LoRA"        # AuraFlow LoRA
CN_DIR      = ROOT / "3_3_AuraFlow_ControlNet"  # AuraFlow ControlNet (手動配置、scan しない)
VAE_DIR     = ROOT / "3_4_AuraFlow_VAE"         # AuraFlow VAE (4ch SDXL系。任意の外部 VAE 上書き用)
EMBED_DIR   = ROOT / "3_5_AuraFlow_Embedding"   # AuraFlow Embedding (pile-T5 ベース)
...
def _classified_destination(kind: str, entry: dict) -> Path:
    ...
    AuraFlow (Pony V7 含む) のみ 3_x の対応レーンへ、それ以外は全て 2_1_errortensors。"""
    if kind in ("broken", "inpainting"):
        return ERROR_DIR
    if kind == "vae":
        # AuraFlow は 4ch SDXL系 VAE を使う (detect_vae_arch は "sd" を返す)。
        # 16ch Flux ae ("flux") は非対応。
        return VAE_DIR if entry.get("vae_arch") == "sd" else ERROR_DIR
    if kind == "controlnet":
        return CN_DIR if entry.get("controlnet_arch") == "auraflow" else ERROR_DIR
    if kind == "base":
        return CKPT_DIR if entry.get("base_arch") == "auraflow" else ERROR_DIR
    if kind == "lora":
        return LORA_DIR if entry.get("lora_arch") == "auraflow" else ERROR_DIR
    if kind == "embedding":
        # AuraFlow embedding (pile-T5 2048) のみ採用。Flux(768/4096)/SDXL(1280) は非対応。
        return EMBED_DIR if entry.get("embedding_arch") == "auraflow" else ERROR_DIR
    return ERROR_DIR
...
def check_tensors() -> dict:
    ...
    # ---- Phase 2a: scan + hash ----
    # ControlNet (3_3) / error (2_1) は scan しない (手動配置 / 退避先)。
    # AuraFlow の base・LoRA・VAE・Embedding dir は scan し、直接投入分も dedup + 再分類する。
    scan_dirs = [TENSORS_DIR, CKPT_DIR, LORA_DIR, VAE_DIR, EMBED_DIR]
    all_files: list[Path] = []
    for d in scan_dirs:
        all_files.extend(d.glob("*.safetensors"))
        all_files.extend(d.glob("*.gguf"))   # GGUF (非対応 → error 行き) も検出だけはする
    all_files.sort()
    ...
    for st, kind, entry, key in survivors:
        ...
        # 退避先 (error) に行ったものは cache から外す (再分類対象にしない、ユーザ手動運用)
    ...
    # AuraFlow LoRA の種別 hint を更新 (ユーザ記入分は保持、hint は再生成)
    update_auraflow_lora_hint_toml()
...
# --------------------------------------------------------------------------- #
# AuraFlow_LoRA_hint.toml (LoRA の種別 subject + ヒント。subject="pose" のみ機能的)
# --------------------------------------------------------------------------- #
AURAFLOW_LORA_HINT_TOML = ROOT / "AuraFlow_LoRA_hint.toml"
_LORA_HINT_NOISE = {"v1", "v2", "v3", "v4", "v5", "v6", "v10", "v20", "v30", "v50",
                    "auraflow", "aura", "pony", "lora", "fp16", "bf16", ""}
...
def update_auraflow_lora_hint_toml() -> int:
    """3_2_AuraFlow_LoRA → AuraFlow_LoRA_hint.toml を更新。"""
    return _write_lora_hint_toml(AURAFLOW_LORA_HINT_TOML, LORA_DIR, "AuraFlow")
...
def update_checkpoint_toml() -> tuple[int, int]:
    """3_1_AuraFlow_checkpoint を scan して checkpoint.toml を追従。
...
def audit_lora_preview_toml() -> tuple[int, int]:
    """LoRA_preview.toml (AuraFlow_categories) と LoRA 実体 (3_2_AuraFlow_LoRA) の過不足を点検。
    ...
    if missing:
        ...
        print(L(f"  [LoRA_preview/AuraFlow] 不足 {len(missing)} 件を ware で追加 ({ex})",
                f"  [LoRA_preview/AuraFlow] added {len(missing)} missing as ware ({ex})"))
    if stale:
        ex = ", ".join(stale[:3]) + (" …" if len(stale) > 3 else "")
        print(L(f"  [LoRA_preview/AuraFlow] 余剰 {len(stale)} 件 (実体なし。--init-categories で整理可): {ex}",
                f"  [LoRA_preview/AuraFlow] {len(stale)} stale entries (no file; tidy via --init-categories): {ex}"))
    if dirty:
...
def main() -> None:
    ...
    ap.add_argument("--refresh-checkpoint-toml", action="store_true",
                    help=L("振り分け処理を行わず、3_1_AuraFlow_checkpoint を scan して checkpoint.toml の "
                           "family / style / score を読み直し・空欄補完のみ実行する",
                           "Skip triage; just rescan 3_1_AuraFlow_checkpoint and refresh family / style / score "
    ...
    print(f"  3_1_AuraFlow_checkpoint : {counts['checkpoint']:4d}")
    print(f"  3_2_AuraFlow_LoRA       : {counts['lora']:4d}")
    print(f"  3_3_AuraFlow_ControlNet : {counts['controlnet']:4d}")
    print(f"  3_4_AuraFlow_VAE        : {counts['vae']:4d}")
    print(f"  3_5_AuraFlow_Embedding  : {counts['embedding']:4d}")
